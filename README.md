...
- **Direct by default.** HTTP library requests use `trust_env=False`; login,
  search, enrichment, and session access start Edge with
  `--no-proxy-server`. A PDF record can opt in to a user-supplied proxy with
  `--proxy-record`; records without a rule remain direct.
...
viewer. PDF transfer timeout is 180 seconds because large authorized files can
legitimately take longer than one minute. A file is recorded as downloaded only
after the response begins with `%PDF-`.

Proxy routing is opt-in per record and defaults to direct:
...
Login, search, enrichment, and session keepalive never use record proxy rules.
...
- Do not bypass paywalls, DRM, CAPTCHA, MFA, rate limits, or account
  entitlements.
- Proxy support is explicit and per PDF record only. Do not use it to evade
  access controls, account entitlements, or local network policy.
